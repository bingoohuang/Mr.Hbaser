...
public abstract class Substituters {
     ...
     * @return 进行值填充后的字符串
     */
    public static String parse(String strVal, Map<Object, Object> props) {
        Set<String> visitedHolders = new HashSet<String>();
     ...
     * @return 进行值填充后的字符串
     */
    public static String parse(String strVal, Map<Object, Object> props, Set<String> visitedHolders,
        ...
        while (startIndex != -1) {
            ...
            if (endIndex != -1) {
                ...
                int defIndex = Strings.lastIndexOf(holder, ":");
                if (defIndex >= 0) {
                    defValue = Strings.trim(holder.substring(defIndex + 1));
                    holder = Strings.trim(holder.substring(0, defIndex));
                }

                if (!visitedHolders.add(holder)) {
                    throw new AppException("Circular PlaceHolder reference '"
                ...
                if (propVal != null) {
                    ...
                    throw new AppException("Could not resolve Placeholder '" + holder + "'");
                }
    ...
    private static int findHolderEndIndex(CharSequence buf, int startIndex) {
        ...
        while (index < buf.length()) {
            if (Strings.substringMatch(buf, index, DEF_HOLDER_SUFFIX)) {
            ...
            else if (Strings.substringMatch(buf, index, DEF_HOLDER_PREFIX)) {
                withinNestedHolder++;
